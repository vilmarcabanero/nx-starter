...
export const useTodoStatsViewModel = (): TodoStatsViewModel => {
  ...
  const stats = useMemo(() => {
    ...
    const highPriority = store.todos.filter(todo => 
      !todo.completed && todo.priority?.level === 'high'
    ).length;
  ...
  const validateFilterValue = useCallback((value: string): value is 'all' | 'active' | 'completed' => {
