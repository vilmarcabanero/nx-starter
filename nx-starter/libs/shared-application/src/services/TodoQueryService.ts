...
export class TodoQueryService implements ITodoQueryService {
  constructor(
    @inject('GetAllTodosQueryHandler') private getAllTodosHandler: GetAllTodosQueryHandler,
    @inject('GetFilteredTodosQueryHandler') private getFilteredTodosHandler: GetFilteredTodosQueryHandler,
    @inject('GetActiveTodosQueryHandler') private getActiveTodosHandler: GetActiveTodosQueryHandler,
    @inject('GetCompletedTodosQueryHandler') private getCompletedTodosHandler: GetCompletedTodosQueryHandler,
    @inject('GetTodoStatsQueryHandler') private getTodoStatsHandler: GetTodoStatsQueryHandler,
    @inject('GetTodoByIdQueryHandler') private getTodoByIdHandler: GetTodoByIdQueryHandler
  ) {}
  ...
  async getTodoStats(): Promise<{ total: number; active: number; completed: number; overdue?: number; highPriority?: number }> {
