...
vi.mock('@/core/infrastructure/todo/persistence/InMemoryTodoRepository', () => ({
...
vi.mock('@/core/infrastructure/todo/persistence/SqliteTodoRepository', () => ({
...
vi.mock('@/core/infrastructure/todo/persistence/typeorm/TypeOrmTodoRepository', () => ({
...
vi.mock('@/core/infrastructure/todo/persistence/mongoose/MongooseTodoRepository', () => ({
...
vi.mock('@/core/infrastructure/todo/persistence/sequelize/SequelizeTodoRepository', () => ({
...
vi.mock('@/core/infrastructure/todo/persistence/typeorm/TypeOrmConnection', () => ({
  getTypeOrmDataSource: vi.fn().mockResolvedValue({ isInitialized: true })
}));

vi.mock('@/core/infrastructure/todo/persistence/mongoose/MongooseConnection', () => ({
  connectMongoDB: vi.fn().mockResolvedValue(undefined)
}));

vi.mock('@/core/infrastructure/todo/persistence/sequelize/SequelizeConnection', () => ({
  getSequelizeInstance: vi.fn().mockResolvedValue({ authenticate: vi.fn() })
}));

// Mock use cases
vi.mock('@/core/application/todo/use-cases/commands/CreateTodoUseCase');
vi.mock('@/core/application/todo/use-cases/commands/UpdateTodoUseCase');
vi.mock('@/core/application/todo/use-cases/commands/DeleteTodoUseCase');
vi.mock('@/core/application/todo/use-cases/commands/ToggleTodoUseCase');
vi.mock('@/core/application/todo/use-cases/queries/TodoQueryHandlers');
...
describe('DI Container Configuration', () => {
  ...
  describe('getRepositoryImplementation - MongoDB', () => {
    it('should return MongooseTodoRepository for mongodb database', async () => {
      const { config } = await import('@/config/config');
      const { connectMongoDB } = await import('@/core/infrastructure/todo/persistence/mongoose/MongooseConnection');
  ...
  describe('getRepositoryImplementation - TypeORM', () => {
    it('should return TypeOrmTodoRepository for typeorm ORM', async () => {
      const { config } = await import('@/config/config');
      const { getTypeOrmDataSource } = await import('@/core/infrastructure/todo/persistence/typeorm/TypeOrmConnection');
  ...
  describe('getRepositoryImplementation - Sequelize', () => {
    it('should return SequelizeTodoRepository for sequelize ORM', async () => {
      const { config } = await import('@/config/config');
      const { getSequelizeInstance } = await import('@/core/infrastructure/todo/persistence/sequelize/SequelizeConnection');
