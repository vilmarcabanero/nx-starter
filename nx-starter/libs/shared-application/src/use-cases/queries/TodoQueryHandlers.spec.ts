...
import type { GetFilteredTodosQuery, GetTodoByIdQuery } from '../../dto/TodoQueries';
...
describe('TodoQueryHandlers', () => {
  ...
  describe('GetFilteredTodosQueryHandler', () => {
    ...
    it('should sort by priority when sortBy is "priority"', async () => {
      ...
      const sortedTodos = [...sampleTodos].sort((a, b) => a.priority.level.localeCompare(b.priority.level));
      vi.spyOn(TodoDomainService, 'sortByPriority').mockReturnValue(sortedTodos);

      // Act
      const result = await handler.execute(query);

      // Assert
      expect(TodoDomainService.sortByPriority).toHaveBeenCalledWith(sampleTodos);
    ...
    it('should sort by createdAt when sortBy is "createdAt"', async () => {
      ...
      expect(result[1].createdAt.getTime()).toBeLessThanOrEqual(result[2].createdAt.getTime());
    });
  });
  ...
  describe('GetTodoByIdQueryHandler', () => {
    it('should return todo when found', async () => {
      ...
      const query: GetTodoByIdQuery = { id: '12345678901234567890123456789026' };
      ...
      expect(mockRepository.getById).toHaveBeenCalledWith('12345678901234567890123456789026');
    });
    ...
    it('should return null when todo not found', async () => {
      ...
      const query: GetTodoByIdQuery = { id: '12345678901234567890123456789999' };
      ...
      expect(mockRepository.getById).toHaveBeenCalledWith('12345678901234567890123456789999');
    });
  });
